...
- **Subprocess per run**: The Python agent runs as a detached child process spawned by Express. It writes events and graph objects directly to PostgreSQL in real-time. stdio is suppressed — errors only visible via DB state.
